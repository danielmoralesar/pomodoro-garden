<div class="card p-4">
    <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post">
        <div class="mb-3">
            ...
            <input type="text" name="name" id="name" class="form-control <?= !empty($errorName) ? "is-invalid" : "" ?>" required>
            <?= !empty($errorName) ? $errorName : ""?>
        </div>
        <div class="mb-3">
            <label for="email" class="form-label">Tu email:</label>
            <input type="email" name="email" id="email" class="form-control <?= !empty($errorEmail) ? "is-invalid" : "" ?>" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" required>
